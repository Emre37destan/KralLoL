...
public:
		...
		std::string cookies, std::string& returnCookies, int port = -1)
	{
		...
		if (header.find(("Accept-Language:")) == std::string::npos)header.append(("Accept-Language: tr-TR\r\n"));
		...
		if (!internetOpen)
			return ("InternetOpenA basarisiz oldu. Son hata: ") + std::to_string(GetLastError());
		...
		if (!internetConnect)
		{
			std::string err = "InternetConnectA basarisiz oldu. Son hata: " + std::to_string(GetLastError());
		...
		if (!openRequest)
		{
			std::string err = "HttpOpenRequestA basarisiz oldu. Son hata: " + std::to_string(GetLastError());
			InternetCloseHandle(internetConnect);
			InternetCloseHandle(internetOpen);
			return err;
		}

		// ssl sertifika hatalarýný yok sayar
		DWORD dwFlags = 0;
		...
		if (!sendRequest)
		{
			std::string err = "HttpSendRequestA basarisiz oldu. Son hata: " + std::to_string(GetLastError());
			InternetCloseHandle(internetConnect);
			InternetCloseHandle(internetOpen);
		...
		{
			std::string err = "Karakter dizisi1 tahsis edilemedi. Son hata: " + std::to_string(GetLastError());
			InternetCloseHandle(internetConnect);
			InternetCloseHandle(internetOpen);
			InternetCloseHandle(openRequest);
			return err;
		...
		{
			std::string err = "Karakter dizisi2 tahsis edilemedi. Son hata: " + std::to_string(GetLastError());
			InternetCloseHandle(internetConnect);
			InternetCloseHandle(internetOpen);
			InternetCloseHandle(openRequest);
			return err;
