...
// YAPILACAKLAR: bunu hareket ettir
...
{
	// Dear ImGui çerçevesini başlatın
...
void Direct3D9Render::EndFrame()
{
	...
	g_pSwapChain->Present(1, 0); // Vsync ile sunun
	//g_pSwapChain->Present(0, 0); // Vsync olmadan mevcut
}

int Direct3D9Render::Render()
{
	char buf[90];
	sprintf_s(buf, ("KralLoL By. Kral - %s \t %s ###AnimatedTitle"), gamePatch.c_str(), champSkins.empty() ? "Veriler indiriliyor..." : "");
...
{
	// Sevgili ImGui bağlamını kurun
	IMGUI_CHECKVERSION();
	ImGui::CreateContext();

	// Sevgili ImGui stilini kurun
	MenuInit();

	// Kurulum Platformu/Renderer arka uçları
...
void Direct3D9Render::MenuInit()
{
	...
	{
		0x0020, 0x00FF, // Temel Latince + Latince Eki
		0x2000, 0x206F, // Genel Noktalama
		0x3000, 0x30FF, // CJK Sembolleri ve Noktalama İşaretleri, Hiragana, Katakana
		0x31F0, 0x31FF, // Katakana Fonetik Uzantıları
		0xFF00, 0xFFEF, // Yarım genişlikli karakterler
		0x4e00, 0x9FAF, // CJK İdeogramları
		0x3131, 0x3163, // Kore alfabeleri
		0xAC00, 0xD7A3, // Korece karakterler
		0x0400, 0x052F, // Kiril + Kiril Eki
		0x2DE0, 0x2DFF, // Kiril Genişletilmiş-A
		0xA640, 0xA69F, // Kiril Genişletilmiş-B
		0x2010, 0x205E, // Noktalama işaretleri
		0x0E00, 0x0E7F, // Tay
		0x0102, 0x0103,
		0x0110, 0x0111,
		0x0128, 0x0129,
