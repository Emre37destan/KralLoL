...
public:
	...
	static void LaunchLegacyClient()
	{
		...
		if (::FindWindowA("RCLIENT", "League of Legends"))
		{
			...
			// müþterinin kapanmasýný bekleyin (belki bunu yapmanýn daha iyi bir yöntemi vardýr)
	...
	static void CheckVersion()
	{
		...
		if (reader->parse(getLatest.c_str(), getLatest.c_str() + static_cast<int>(getLatest.length()), &root, &err))
		{
			...
			if (latestName != "1.9.0")
			{
				if (MessageBoxA(0, "Ýndirme sitesi açýlsýn mý?", "Yeni sürüm mevcut", MB_YESNO | MB_SETFOREGROUND) == IDYES)
				{
	...
	static std::string ChampIdToName(int id)
	{
		...
		{
			return "Veri yok";// "Þampiyon verileri hala indiriliyor";
		}
		{
	...
	// Üzerine gelindiðinde bir araç ipucunu gösteren küçük bir (?) iþareti görüntülemek için yardýmcý.
	// Birleþtirilmiþ simge yazý tipleri kullanýyorsanýz, kendi kodunuzda gerçek bir simge görüntülemek isteyebilirsiniz (bkz. docs/FONTS.md)
	...
	// Ýþlev baþarýlý olursa, dönüþ deðeri sýfýr deðildir.
