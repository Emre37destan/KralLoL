...
public:
	static void Render()
	{
		...
		if (ImGui::BeginTabItem("Sampiyonlar"))
		{
			...
			ImGui::SameLine();
			ImGui::RadioButton("Ustalik", &iSort, 2);
			ImGui::SameLine();
					...
					if (reader->parse(getChampions.c_str(), getChampions.c_str() + static_cast<int>(getChampions.length()), &root, &err))
					{
						if (root.isArray())
						{
							for (Json::Value::ArrayIndex i = 0; i < root.size(); ++i)
							{
								...
								//auto &rolesObj = champObj.GetObject("roles"); //yapmak
			...
			ImGui::Separator();
			ImGui::Text("Sahip olunan sampiyonlar: %d", iChampsOwned);
