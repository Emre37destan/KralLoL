...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	{
		// Geçerli saati kullanarak rastgele yapın, son c++ rastgele ile takas yapın
		...
		// Pencere sınıfı bilgilerini kaydet
		...
		if (hwnd == NULL)
		{
			...
			MessageBoxA(0, "Pencere olusturulamadi", 0, 0);
			return 0;
		}
		...
		if (!Direct3D9.DirectXInit(hwnd))
		{
			Direct3D9.Shutdown();
			::UnregisterClassA(wc.lpszClassName, wc.hInstance);
			MessageBoxA(0, "DirectX baslatilamadi", 0, 0);
			return 0;
		}
		...
		{
			// Lig istemcisi çalışıyor
		}
		else
		{
			// Giriş ekranı ile isyan istemcisi açıldı
		...
		while (msg.message != WM_QUIT)
		{
			...
			// Orijinal boyutuna sıfırlamak için düğmeye basıldı
			if (S.Window.resize)
			{
			...
			if (oldDebugger != S.debugger)
			{
				...
				if (regCreate == ERROR_SUCCESS)
				{
					...
					else if (regQuery == ERROR_FILE_NOT_FOUND && oldDebugger) // anahtar yoksa, oluşturun
					{
						if (RegSetValueExA(hkResult, "debugger", 0, REG_SZ, (const BYTE*)filePath, len) == ERROR_SUCCESS)
						{
			...
			// Oluşturmayı sonlandır
			Direct3D9.EndFrame();

			// İstemci kapanırsa boşta ve yeniden bağlanın
...
// İleti bildirim işleyicisini imgui_impl_win32.cpp'den ilet
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam);

// Win32 mesaj işleyicisi
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		return 0;
	case WM_SYSCOMMAND:
		if ((wParam & 0xfff0) == SC_KEYMENU) // ALT uygulama menüsünü devre dışı bırak
			return 0;
		break;
	case WM_DESTROY:
		::PostQuitMessage(0);
