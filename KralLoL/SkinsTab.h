...
public:
	static void Render()
	{
		...
		if (ImGui::BeginTabItem("Kostumler"))
		{
			ImGui::Text("Siralamalar: ");
			ImGui::SameLine();

			static int iSort = 0;
			static short iLastSort = -1;
			ImGui::RadioButton("Alfabetik", &iSort, 0);
			ImGui::SameLine();
			ImGui::RadioButton("Alim", &iSort, 1);
			ImGui::SameLine();
			...
			ImGui::Separator();
			ImGui::Text("Sahip olunan gorunumler: %d", ownedSkins.size());

			for (Skin skin : ownedSkins)
			{
				ImGui::Separator();
				if (!skin.name.empty())
					ImGui::Text("Ismi: %s", skin.name.c_str());
				//else if (!champSkins.empty())
				//	ImGui::Text("name: Chroma"); // chroma'nýn adýný almanýn bir yolunu bulmak için
				ImGui::Text("envanterTuru: %s", skin.inventoryType.c_str());
				ImGui::Text("itemId: %d", skin.itemId);
				ImGui::Text("MulkiyetTuru: %s", skin.ownershipType.c_str());
				ImGui::Text("bagbozumu: %d", skin.isVintage);

				char timeBuff[50];
				strftime(timeBuff, sizeof(timeBuff), "%G-%m-%d %H:%M:%S", &skin.purchaseDate);
				ImGui::Text("Alindi: %s", timeBuff);

				ImGui::Text("miktar: %d", skin.qunatity);
